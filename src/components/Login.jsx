...
const Login = ({ type, onLogin }) => {
  ...
  }, [type, setEmail, setPassword, setSelectedRole]);
  ...
  const handleFormSubmit = (e) => {
    ...
    if (selectedRole) {
      ...
      userEmail = selectedRole.email;
    } else {
      const cred = testCredentials.find((c) => c.email === email);
      if (cred) roleToLogin = cred.role;
    }
    
    if (onLogin) onLogin({ email: userEmail, role: roleToLogin, name: userEmail.split('@')[0] });
  };
        ...
        {type === 'como-funciona' ? (
          ...
          <>
            ...
            <>
              ...
              {type !== 'forgot' && (<div><div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '6px' }}><label style={{ fontSize: '0.85rem', fontWeight: '600', color: '#475569' }}>Contraseña</label>{type === 'login' && <span onClick={() => navigate('/forgot')} style={{ fontSize: '0.8rem', color: '#00d084', fontWeight: '600', cursor: 'pointer', transition: 'color 0.2s' }} onMouseOver={(e) => e.currentTarget.style.color = '#059669'} onMouseOut={(e) => e.currentTarget.style.color = '#00d084'}>¿Olvidaste tu contraseña?</span>}</div><input className="modal-input" type="password" required value={password} onChange={(e) => setPassword(e.target.value)} placeholder="••••••••" style={{ width: '100%', padding: '12px 16px', borderRadius: '12px', border: '1px solid #cbd5e1', fontSize: '1rem', outline: 'none', transition: 'border-color 0.2s', boxSizing: 'border-box' }} onFocus={(e) => e.currentTarget.style.borderColor = '#00d084'} onBlur={(e) => e.currentTarget.style.borderColor = '#cbd5e1'} /></div>)}
            </>
          )}
              <button type="submit" style={{ backgroundColor: '#0f172a', color: 'white', padding: '14px', borderRadius: '12px', border: 'none', fontWeight: '700', fontSize: '1rem', cursor: 'pointer', marginTop: '8px', transition: 'background-color 0.2s', boxShadow: '0 4px 6px -1px rgba(15, 23, 42, 0.2)' }} onMouseOver={(e) => e.currentTarget.style.backgroundColor = '#1e293b'} onMouseOut={(e) => e.currentTarget.style.backgroundColor = '#0f172a'}>{selectedRole ? `Entrar como ${selectedRole.role}` : type === 'login' ? 'Iniciar Sesión' : 'Enviar Enlace'}</button>
            </form>
